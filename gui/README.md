...
- [x] **DiffView** — unified/split diff renderer: file header, hunk headers, +/- line
  coloring, line numbers. THE backbone (ROADMAP C.1). Reused by working-changes,
  history, commit detail, conflict preview, PR review.
- [x] **FileStatusRow** (inline in Changes; extract pending) — path + status badge + per-file actions (stage/unstage/discard).
  Used in working-changes and commit detail.
- [x] **CommitRow** (inline in History; extract pending) — short hash + subject + author + relative time. Used in history,
  branch view, PR commits.
- [x] **BranchItem** (inline in Branches; extract pending) — name + current marker + ahead/behind + row actions.
- [x] **CheckBadge** — check name + colored status. (exists inline; extract it)
- [x] **StatusBadge** — the added/modified/deleted/conflict letter+color chip.
- [x] **ConfirmButton** (confirm dialogs for discard/branch-delete/stash-drop) — standard button; ConfirmButton for destructive
  ops (reset --hard, force push, branch/stash delete). (ROADMAP C.3)
- [x] **Toast** — success/error (exists; keep).
...
- [x] spacing scale (tight baseline)
...
- [x] neutral chrome tokens (bg / panel / border / muted / fg), light + dark
