...
export function ConfirmDialog({
  title, body, confirmLabel, onConfirm, onCancel, busy, danger = true, extra,
}: {
  title: string;
  body: ReactNode;
  confirmLabel: string;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-96 rounded-lg border border-border bg-card p-4 shadow-lg">
        <div className={`mb-2 text-sm font-semibold ${danger ? "text-[var(--color-removed)]" : ""}`}>{title}</div>
        ...
        <div className="flex justify-end gap-2">
          ...
          {extra && (
            <button onClick={extra.onClick} disabled={busy}
                    className="rounded-md border border-border px-3 py-1.5 text-sm hover:bg-muted disabled:opacity-40">{extra.label}</button>
          )}
