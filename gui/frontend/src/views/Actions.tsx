...
export function Actions() {
  ...
  const loadRuns = async () => {
    setBusy("runs-load");
    try {
      setRuns((await service.ListRuns(50)) ?? []);
    } catch (e) {
      flash("err", String(e));
    } finally {
      setBusy("");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = (await service.ListRuns(50)) ?? [];
        if (!cancelled) setRuns(r);
        const wfs = (await service.ListDispatchableWorkflows()) ?? [];
        if (!cancelled) setDispatchable(wfs);
      } catch (e) {
  ...
  const loadJobs = async (runID: number) => {
    try {
      setJobs((await service.RunJobs(runID)) ?? []);
    } catch (e) {
      flash("err", String(e));
    }
  };
  const showFlow = async (runID: number) => {
    setRunViewMode("flow");
    try {
      setJobGraph((await service.RunJobGraph(runID)) ?? []);
    } catch (e) {
      flash("err", String(e));
    }
  };
  ...
  const doCancel = (r: WorkflowRun) =>
    run(
      `cancel-${r.ID}`,
      async () => {
        await service.CancelRun(r.ID);
        await loadRuns();
        return `cancelled run #${r.Number}`;
      },
      "cancelled",
    );
  const doRerun = (r: WorkflowRun, failedOnly: boolean) =>
    run(
      `rerun-${r.ID}`,
      async () => {
        if (failedOnly) {
          await service.RerunFailed(r.ID);
        } else {
          await service.RerunRun(r.ID);
        }
        await loadRuns();
        return `re-running #${r.Number}`;
      },
      "re-run started",
    );
  ...
  const toggleLogs = async (jobID: number) => {
    ...
    setBusy(`logs-${jobID}`);
    try {
      setJobLog(await service.JobLogs(jobID));
      setOpenLog(jobID);
    } catch (e) {
      flash("err", String(e));
    } finally {
  ...
  for (const r of filtered) {
    ...
    if (!groups.has(key))
      groups.set(key, { id: r.WorkflowID, name: r.WorkflowName || r.Name || "workflow", runs: [] });
