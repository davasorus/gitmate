  ...
  const doMerge = (b: string) =>
    run(
      ...
      async () => {
        ...
        return c.length ? `merge hit ${c.length} conflict(s) — see banner` : `merged ${b}`;
      },
      `merged ${b}`,
    );
  const doRebase = (b: string) =>
    run(
      ...
      async () => {
        ...
        const c = (await service.ConflictedFiles()) ?? [];
        return c.length ? `rebase hit ${c.length} conflict(s) — see banner` : `rebased onto ${b}`;
      },
      `rebased onto ${b}`,
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 rounded-lg border border-border p-3">
        ...
        />
        <button onClick={doCreate} disabled={!!busy || !newBranch.trim()} className={cls.btn}>
      ...
      <div className="rounded-lg border border-border">
        {(branches ?? []).map((b) => (
          ...
          >
            {b.IsCurrent ? (
              <span className="text-[var(--color-added)]">●</span>
            ) : (
              <span className="w-2" />
            )}
            <b>{b.Name}</b>
            ...
            <span className="ml-auto flex shrink-0 gap-1">
              ...
              {!b.IsCurrent && (
                <button onClick={() => doMerge(b.Name)} disabled={!!busy} className={cls.btnSm}>
                  {busy === `merge-branch-${b.Name}` ? "…" : "Merge"}
                </button>
              )}
              {!b.IsCurrent && (
                <button onClick={() => doRebase(b.Name)} disabled={!!busy} className={cls.btnSm}>
