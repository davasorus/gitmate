...
export function Changes() {
  ...
  const showDiff = async (path: string, staged: boolean) => {
    if (openDiff && openDiff.path === path && openDiff.staged === staged) {
      setOpenDiff(null); setDiffFiles([]); return;
    }
    setBusy(`diff-${path}`);
    try {
  ...
  const doUnstage = (p: string) => run(`unstage-${p}`, () => service.UnstagePath(p), `unstaged ${p}`);
  const doDiscard = (p: string) => run(`discard-${p}`, async () => { await service.DiscardPath(p); setConfirmDiscard(null); return `discarded ${p}`; }, `discarded ${p}`);
  const doCommit = () => run("commit", async () => { await service.Stage(); const h = await service.Commit(commitMsg); setCommitMsg(""); return `committed ${h}`; }, "committed");
  ...
  const Row = ({ path, isStaged, badge }: { path: string; isStaged: boolean; badge: React.ReactNode }) => (
    <div>
      <div className="flex items-center gap-2 border-b border-border px-3 py-1.5 text-sm last:border-0 hover:bg-muted/60">
        <button onClick={() => (isStaged ? doUnstage(path) : doStage(path))} disabled={!!busy}
        ...
        {!isStaged && (
          <button onClick={() => setConfirmDiscard(path)} disabled={!!busy}
        ...
        <button onClick={() => showDiff(path, isStaged)} className="flex flex-1 items-baseline gap-2 text-left">
          {badge}<span className="truncate">{path}</span>
          {busy === `diff-${path}` && <span className="ml-auto text-xs text-muted-foreground">…</span>}
        </button>
      </div>
      {openDiff?.path === path && openDiff.staged === isStaged && (
        <div className="border-b border-border bg-background px-2 py-2"><DiffView files={diffFiles} /></div>
      )}
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="rounded-lg border border-border">
          {staged.length === 0 ? <div className="p-3 text-xs italic text-muted-foreground">nothing staged</div>
            : staged.map((c) => <Row key={`s-${c.Path}`} path={c.Path} isStaged badge={<StatusBadge staged={c.Staged} unstaged="" />} />)}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="rounded-lg border border-border">
          {unstaged.length === 0 && untracked.length === 0 ? <div className="p-3 text-xs italic text-muted-foreground">nothing unstaged</div>
            : <>
                {unstaged.map((c) => <Row key={`u-${c.Path}`} path={c.Path} isStaged={false} badge={<StatusBadge staged="" unstaged={c.Unstaged} />} />)}
                {untracked.map((u) => (
      ...
      <div className="flex gap-2">
        <input value={commitMsg} onChange={(e) => setCommitMsg(e.target.value)} placeholder="commit message (stages all)" className={`${cls.input} flex-1`} />
        <button onClick={doCommit} disabled={!!busy || !commitMsg.trim()} className={cls.btn}>{busy === "commit" ? "…" : "Commit"}</button>
      </div>

      {confirmDiscard && (
        <ConfirmDialog title="Discard changes?"
          body={<>This permanently deletes your uncommitted changes to <span className="text-foreground">{confirmDiscard}</span>. This cannot be undone.</>}
          confirmLabel="Discard" busy={!!busy}
          onCancel={() => setConfirmDiscard(null)} onConfirm={() => doDiscard(confirmDiscard)} />
      )}
    </div>
  );
}
