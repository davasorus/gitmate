...
export function Conflicts() {
  ...
  const view = async (path: string) => {
    ...
    try {
      setDetail(await service.ReadConflict(path));
      setOpen(path);
    } catch (e) {
      flash("err", String(e));
    } finally {
  ...
  const takeOurs = (p: string) =>
    run(
      `ours-${p}`,
      async () => {
        await service.ResolveOurs(p);
        setOpen(null);
        return `took ours for ${p}`;
      },
      "resolved",
    );
  const takeTheirs = (p: string) =>
    run(
      `theirs-${p}`,
      async () => {
        await service.ResolveTheirs(p);
        setOpen(null);
        return `took theirs for ${p}`;
      },
      "resolved",
    );
  const markResolved = (p: string) =>
    run(
      `mark-${p}`,
      async () => {
        await service.MarkResolved(p);
        setOpen(null);
        return `marked ${p} resolved`;
      },
      "resolved",
    );
            ...
            <div key={path}>
              <div className="flex items-center gap-2 border-b border-border px-3 py-1.5 text-sm last:border-0">
                ...
                <span className="flex shrink-0 gap-1">
                  <button onClick={() => takeOurs(path)} disabled={!!busy} className={cls.btnSm}>
                    {busy === `ours-${path}` ? "…" : "Take ours"}
                  </button>
                  <button onClick={() => takeTheirs(path)} disabled={!!busy} className={cls.btnSm}>
                    {busy === `theirs-${path}` ? "…" : "Take theirs"}
                  </button>
                  <button
                ...
                <div className="space-y-2 border-b border-border bg-background px-3 py-2">
                  {(detail.Hunks ?? []).length === 0 && (
                    <div className="text-xs italic text-muted-foreground">
                      no conflict regions found (file may already be resolved or hand-edited)
                    </div>
                  )}
                  {(detail.Hunks ?? []).map((h, i) => (
                    <div key={i} className="grid grid-cols-2 gap-2 text-xs">
                      <div className="rounded border border-[var(--color-added)]/40 bg-[var(--color-added)]/5 p-2">
                        <div className="mb-1 font-semibold text-[var(--color-added)]">
                      ...
                      <div className="rounded border border-[var(--color-ahead)]/40 bg-[var(--color-ahead)]/5 p-2">
                        <div className="mb-1 font-semibold text-[var(--color-ahead)]">
                          theirs — merged-in branch
                        </div>
