...
import type { Commit, CommitDetail } from "../../bindings/github.com/davasorus/gitmate/internal/gitops";
...
export function History() {
  ...
  const loadBranch = async (ref: string) => {
    setViewBranch(ref);
    if (ref === "" || ref === current) { setRefCommits(null); return; }
    setBusy("history-branch");
    try { setRefCommits((await service.LogRef(ref, 30)) ?? []); }
    catch (e) { flash("err", String(e)); } finally { setBusy(""); }
  };

  // reset to current branch when it changes (e.g. after a switch)
  useEffect(() => { setViewBranch(""); setRefCommits(null); }, [current]);
  ...
  const doCherryPick = (hash: string) =>
    run(`cherry-${hash}`, async () => {
      ...
      return c.length ? `cherry-pick hit ${c.length} conflict(s) — see banner` : `cherry-picked ${hash.slice(0,7)} onto ${current}`;
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <select value={viewBranch || current} onChange={(e) => loadBranch(e.target.value)} className={cls.input}>
            {(branches ?? []).map((b) => (
      ...
      {viewingOther && (
        <div className="rounded-md border border-[var(--color-ahead)]/40 bg-[var(--color-ahead)]/5 px-3 py-1.5 text-xs text-muted-foreground">
          Viewing <span className="text-[var(--color-ahead)]">{viewBranch}</span>. Cherry-pick / revert here apply onto your current branch <span className="text-[var(--color-added)]">{current}</span>.
        </div>
      )}

      <div className="rounded-lg border border-border">
        {busy === "history-branch" ? <div className="p-3 text-sm text-muted-foreground">…</div>
          : shown.length === 0 ? <div className="p-3 text-sm italic text-muted-foreground">no commits</div>
          : shown.map((c) => (
          <div key={c.Hash}>
            <div className="flex items-center gap-2 border-b border-border px-3 py-1.5 text-sm last:border-0 hover:bg-muted/60">
              ...
              <span className="flex shrink-0 gap-1">
                <button onClick={() => doCherryPick(c.Hash)} disabled={!!busy} className="rounded-md border border-border px-2 py-0.5 text-xs hover:bg-muted disabled:opacity-40" title={`apply this commit onto ${current}`}>
