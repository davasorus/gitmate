  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-border">
        {busy === "issues-load" ? <div className="p-3 text-sm text-muted-foreground">…</div>
          : (issues ?? []).length ? (issues ?? []).map((i) => (
          <div key={i.Number} className="border-b border-border px-3 py-1.5 last:border-0">
            <div className="flex items-center gap-2 text-sm">
              <span className="shrink-0 font-semibold text-[var(--color-ahead)]">#{i.Number}</span>
              <span className="truncate">{i.Title}</span>
              <span className="text-xs text-muted-foreground">@{i.Author}</span>
              <span className="ml-auto flex shrink-0 gap-1">
                <button onClick={() => doClose(i.Number)} disabled={!!busy} className={`${cls.btnSm} text-[var(--color-removed)] hover:bg-[var(--color-removed)]/10`}>{busy === `issue-close-${i.Number}` ? "…" : "Close"}</button>
                <button onClick={() => doReopen(i.Number)} disabled={!!busy} className={cls.btnSm}>{busy === `issue-reopen-${i.Number}` ? "…" : "Reopen"}</button>
              </span>
            </div>
            <div className="mt-1 flex flex-wrap items-center gap-1">
              {(i.Labels ?? []).map((l) => (
                <button key={l} onClick={() => doRemoveLabel(i.Number, l)} disabled={!!busy}
                        className="rounded-full border border-border px-2 py-0.5 text-[10px] hover:bg-[var(--color-removed)]/10" title="click to remove">
                  {l} ✕
                </button>
              ))}
              <input value={labelInput[i.Number] ?? ""} onChange={(e) => setLabelInput((m) => ({ ...m, [i.Number]: e.target.value }))}
                     onKeyDown={(e) => { if (e.key === "Enter" && (labelInput[i.Number] ?? "").trim()) doAddLabel(i.Number, labelInput[i.Number].trim()); }}
