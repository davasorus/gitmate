...
export function Releases() {
  ...
  const reload = async () => {
    ...
    try {
      setReleases((await service.ListReleases()) ?? []);
    } catch (e) {
      flash("err", String(e));
    } finally {
      setBusy("");
  ...
  useEffect(() => {
    ...
    (async () => {
      setBusy("releases-load");
      try {
        const r = (await service.ListReleases()) ?? [];
  ...
  const loadAssets = async (releaseID: number) => {
    ...
    try {
      setAssets((await service.ListAssets(releaseID)) ?? []);
      setOpenAssets(releaseID);
    } catch (e) {
      flash("err", String(e));
    } finally {
      setBusy("");
  ...
  const doDelete = (r: Release) => {
    ...
    run(
      `release-del-${r.ID}`,
      async () => {
        await service.DeleteRelease(r.ID);
        setConfirmDel(null);
        await reload();
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2 rounded-lg border border-border p-3">
        <div className="text-xs text-muted-foreground">
          Create a release on a tag. Use Generate notes to auto-fill from merged PRs/commits.
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          />
          <button onClick={doGenNotes} disabled={!!busy || !tag.trim()} className={cls.btnSm}>
            {busy === "gen-notes" ? "…" : "Generate notes"}
          </button>
        </div>
        <textarea
          value={body}
        ...
        <div className="flex items-center gap-4 text-xs">
          ...
          <button
            ...
            className={`${cls.btn} ml-auto`}
          >
            {busy === "release-create" ? "…" : "Create release"}
          </button>
        </div>
      </div>
          ...
          (releases ?? []).map((r) => (
            <div key={r.ID} className="border-b border-border last:border-0">
              <div className="flex items-center gap-2 px-3 py-1.5 text-sm">
                <span className="shrink-0 font-semibold text-[var(--color-modified)]">
                  {r.TagName}
                </span>
                <span className="truncate">{r.Name}</span>
                {r.Draft && (
                  <span className="shrink-0 rounded bg-border px-1 text-[10px] uppercase">
                    draft
                  </span>
                )}
                {r.Prerelease && (
                  <span className="shrink-0 rounded bg-[var(--color-behind)]/20 px-1 text-[10px] uppercase text-[var(--color-behind)]">
                    pre
                  </span>
                )}
                {r.Immutable && (
                  <span
                    className="shrink-0 rounded bg-[var(--color-ahead)]/20 px-1 text-[10px] uppercase text-[var(--color-ahead)]"
                    title="immutable: assets and this release cannot be added, changed, or deleted"
                  >
                    🔒 immutable
                  </span>
                )}
                <span className="ml-auto flex shrink-0 gap-1">
                  <button
                    onClick={() => toggleAssets(r.ID)}
                    disabled={!!busy}
                    className={cls.btnSm}
                  >
                    {busy === `assets-${r.ID}` ? "…" : "Assets"}
                  </button>
                  <a href={r.URL} target="_blank" rel="noreferrer" className={cls.btnSm}>
                    View
                  </a>
                  <button onClick={() => setEditing(r)} disabled={!!busy} className={cls.btnSm}>
                    Edit
                  </button>
                  <button
                ...
                <div className="space-y-1 bg-background px-3 pb-2 pt-1">
                      ...
                      <div key={a.ID} className="flex items-center gap-2 text-xs">
                        ...
                        <span className="ml-auto flex shrink-0 gap-1">
                          <button
                            ...
                            className={cls.btnSm}
                          >
                            {busy === `asset-dl-${a.ID}` ? "…" : "Download"}
                          </button>
                          <button
                            onClick={() => onDeleteAsset(r, a)}
                            disabled={!!busy || r.Immutable}
                  ...
                  {r.Immutable ? (
                    <div className="text-[11px] italic text-muted-foreground">
                      🔒 This release is immutable — assets cannot be added or removed.
                    </div>
                  ) : (
      ...
      {confirmDel && (
        <ConfirmDialog
          ...
          body={
            <>
              Delete the release for <span className="text-foreground">{confirmDel.TagName}</span>?
              The underlying tag is NOT deleted — only the release.
            </>
          }
          confirmLabel="Delete"
          busy={!!busy}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-[32rem] space-y-3 rounded-lg border border-border bg-card p-4 shadow-lg">
            ...
            <textarea
              ...
              placeholder="notes"
              className={`${cls.input} h-40 w-full resize-y`}
            />
            <div className="flex items-center gap-4 text-xs">
              <label className="flex items-center gap-1">
                <input
              ...
              <label className="flex items-center gap-1">
                <input
                  type="checkbox"
                  checked={editing.Prerelease}
                  onChange={(e) => setEditing({ ...editing, Prerelease: e.target.checked })}
                />{" "}
                prerelease
              </label>
            </div>
            <div className="flex justify-end gap-2">
              ...
              <button onClick={doEdit} disabled={!!busy} className={cls.btn}>
                {busy === "release-edit" ? "…" : "Save"}
              </button>
            </div>
          </div>
        </div>
