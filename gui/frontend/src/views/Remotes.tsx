...
export function Remotes() {
  ...
  const load = async () => {
    ...
    try {
      setRemotes((await service.ListRemotes()) ?? []);
    } catch (e) {
      flash("err", String(e));
    } finally {
      setBusy("");
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2 rounded-lg border border-border p-3">
        <div className="text-xs text-muted-foreground">
          Clone a repository. Leave destination blank to use a folder named after the repo. On
          success the app switches to the clone.
        </div>
        <div className="flex flex-wrap gap-2">
          <input
            value={cloneUrl}
            onChange={(e) => setCloneUrl(e.target.value)}
            placeholder="repo URL (https://github.com/owner/repo.git)"
          ...
          />
          <button onClick={doClone} disabled={!!busy || !cloneUrl.trim()} className={cls.btn}>
            {busy === "clone" ? "…" : "Clone"}
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 rounded-lg border border-border p-3">
        ...
        <input
          ...
          placeholder="url (https://github.com/owner/repo.git)"
          className={`${cls.input} flex-1`}
        />
        <button
          onClick={doAdd}
      ...
      {confirmDel && (
        <ConfirmDialog
          ...
          body={
            <>
              Remove remote <span className="text-foreground">{confirmDel}</span>? This only drops
              the local reference — commits are untouched, and you can re-add it.
            </>
          }
          confirmLabel="Remove"
          busy={!!busy}
